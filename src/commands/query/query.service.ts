...
export type PortfolioSummary = {
	account: AccountScopedParams["account"];
	holdings: TossInvestApiResponse<HoldingsOverview>;
};
...
export interface IQueryCommandService {
	 ...
	 * @returns 계정 목록 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	accounts(): Promise<TossInvestApiResponse<Account[]>>;
	 ...
	 * @returns 구매력 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	buyingPower(params: GetBuyingPowerParams): Promise<TossInvestApiResponse<BuyingPowerResponse>>;
	 ...
	 * @returns 캔들 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	candles(params: GetCandlesParams): Promise<TossInvestApiResponse<CandlePageResponse>>;
	 ...
	 * @returns 수수료 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	commissions(params: AccountScopedParams): Promise<TossInvestApiResponse<Commission[]>>;
	 ...
	 * @returns 조건부 주문 단건 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	conditionalOrder(params: ConditionalOrderIdentityParams): Promise<TossInvestApiResponse<ConditionalOrderDetailResponse>>;
	 ...
	 * @returns 조건부 주문 목록 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	conditionalOrders(params: GetConditionalOrdersParams): Promise<TossInvestApiResponse<PaginatedConditionalOrderResponse>>;
	 ...
	 * @returns 환율 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	exchangeRate(params: GetExchangeRateParams): Promise<TossInvestApiResponse<ExchangeRateResponse>>;
	 ...
	 * @returns 보유 종목 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	holdings(params: GetHoldingsParams): Promise<TossInvestApiResponse<HoldingsOverview>>;
	 ...
	 * @returns 한국 시장 캘린더 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	krMarketCalendar(params?: GetMarketCalendarParams): Promise<TossInvestApiResponse<KrMarketCalendarResponse>>;
	 ...
	 * @returns 시장 지표 캔들 API의 응답 결과(구체적인 응답 envelope).
	 */
	 ...
	 * @returns 투자자 트레이딩 지표 API의 응답 결과(구체적인 응답 envelope).
	 */
	 ...
	 * @returns 시장 가격 지표 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	 ...
	 * @returns 주문 상세 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	order(params: OrderIdentityParams): Promise<TossInvestApiResponse<Order>>;
	 ...
	 * @returns 주문호가 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	orderbook(params: GetOrderbookParams): Promise<TossInvestApiResponse<OrderbookResponse>>;
	 ...
	 * @returns 주문 목록 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	orders(params: GetOrdersParams): Promise<TossInvestApiResponse<PaginatedOrderResponse>>;
	 ...
	 * @returns `{ account, holdings }` 형태의 포트폴리오 요약 래퍼(구체적인 응답 envelope).
	 */
	portfolioSummary(params: AccountScopedParams): Promise<PortfolioSummary>;
	 ...
	 * @returns 가격 제한 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	priceLimit(params: GetPriceLimitParams): Promise<TossInvestApiResponse<PriceLimitResponse>>;
	 ...
	 * @returns 다종목 가격 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	prices(params: GetPricesParams): Promise<TossInvestApiResponse<PriceResponse[]>>;
	 ...
	 * @returns 랭킹 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	rankings(params: GetRankingsParams): Promise<TossInvestApiResponse<RankingResponse>>;
	 ...
	 * @returns 매도 가능 수량 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	sellableQuantity(params: GetSellableQuantityParams): Promise<TossInvestApiResponse<SellableQuantityResponse>>;
	 ...
	 * @returns 종목 경고 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	stockWarnings(params: GetStockWarningsParams): Promise<TossInvestApiResponse<StockWarning[]>>;
	 ...
	 * @returns 종목 정보 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	stocks(params: GetStocksParams): Promise<TossInvestApiResponse<StockInfo[]>>;
	 ...
	 * @returns 체결 이력 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	trades(params: GetTradesParams): Promise<TossInvestApiResponse<Trade[]>>;
	 ...
	 * 응답을 그대로 반환합니다.
	 * @param params 조회 기간 또는 상세 필터를 담는 선택적 요청 파라미터.
	 * @returns 미국 시장 캘린더 조회 API의 응답 결과(구체적인 응답 envelope).
	 */
	usMarketCalendar(params?: GetMarketCalendarParams): Promise<TossInvestApiResponse<UsMarketCalendarResponse>>;
}
...
export class QueryCommandService implements IQueryCommandService {
	...
	async portfolioSummary(params: AccountScopedParams): Promise<PortfolioSummary> {
